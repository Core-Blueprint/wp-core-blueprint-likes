...
/** Builder-neutral read contract for reaction data. */
final class Data {
	...
	public static function value( string $field, mixed $context = null ): string|int|null {
		...
		$dislikes = Settings::dislike_enabled_for_target( $target['type'], $target['id'] );
		$user_id  = get_current_user_id();

		return match ( $field ) {
			self::LIKE_COUNT => Repository::count( $target['type'], $target['id'] ),
			self::DISLIKE_COUNT => $dislikes ? Repository::dislike_count( $target['type'], $target['id'] ) : 0,
			self::HAS_LIKED => $user_id > 0 && Repository::user_has_liked( $user_id, $target['type'], $target['id'] ) ? '1' : '0',
			self::HAS_DISLIKED => $dislikes && $user_id > 0 && Repository::user_has_disliked( $user_id, $target['type'], $target['id'] ) ? '1' : '0',
			default => null,
		};
	}
}
